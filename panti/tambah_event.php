      ...
      <div class="container row3">
        <div class="row">
            ...
            <div class="ml-md-5 col-ex-5 text-uppercase">
              ...
              <form method="POST" action="proses/tambah_event_config.php"  enctype="multipart/form-data">
                          <?php

                          

                          include "../koneksi.php";
                              $query = mysqli_query($host, "SELECT max(id_event) as idTerbesar FROM event");
                              $data = mysqli_fetch_array($query);
                              $id_event = $data['idTerbesar'];  
                              
                              $urutan = (int) substr($id_event, 3, 3);
                              $urutan++;
                              $huruf = "EVN";
                              $id_donasi = $huruf . sprintf("%03s", $urutan);
                              $date = date("d-m-Y");
                              $nama_panti = $_GET['nama_panti'];
                              $query_mysql = mysqli_query($host, "SELECT * FROM table_panti  WHERE nama_panti ='$nama_panti'");
                              $data1 = mysqli_fetch_array($query_mysql);
                          ?>
                                <div class="input-group mb-3">
                                  <span class="input-group-text" id="basic-addon1">Nama Event</span>
                                  <input type="hidden" class="form-control form-control-user" id="id" name="id_event" value="<?php echo $id_donasi ?>" readonly>
                                ...
                                <span class="input-group-text" id="basic-addon1">Judul Event</span>                     
                                  <input class="form-control" type="text" name="judul_event" >
                                </div>
                                ...
                                <div class="input-group mb-3">
                                  ...
                                  <input class="form-control" type="date"  name="tanggal_event" >
                                </div>

                                <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon1">Deskripsi Event</span>                     
                                  <input class="form-control" type="text" name="deskripsi_event" >
                                </div>
                                ...
                                <span class="input-group-text" id="basic-addon1">Isi Event</span>                     
                                  <textarea class="form-control"  name="isi_event" ></textarea>
                                </div>
